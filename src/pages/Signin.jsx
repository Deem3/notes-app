    ...
    <div className='h-screen flex justify-center items-center'>
      <div className='bg-white rounded-xl shadow w-96 h-96'>
        <div className='flex flex-col items-center'>
          ...
          <p className='font-lobster text-green-500 text-2xl'>Signin</p>
          </div>
            <form onSubmit={handleSubmit}>
              ...
              <button disabled={isLoading} className='bg-gray-300 rounded-lg shadow p-2 w-64 hover:bg-gray-200 hover:text-green-500 transition-all'>Signin</button>
              {err && <div>{err}</div>}
              <Link className='flex justify-center hover:text-green-500' to="/signup">To Signup</Link>
              </div>
            </form>
        </div>
      </div>
